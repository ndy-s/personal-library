            ...
            <div class="flex flex-col xl:flex-row justify-between items-end xl:items-center px-4 xl:px-0 xl:ml-12 xl:mr-0 mb-5">
                ...
                <div class="flex justify-between items-center w-full xl:w-[70%] xl:mr-4">
                    <h1 class="font-bold text-xl lg:text-3xl text-gray-200">
                        Art Library <span class="text-xs lg:text-sm text-gray-400 align-top">(Total {{ localLibraryTotal ?? 0 }})</span>
                    </h1>
                    <div class="flex items-center gap-3">
                        <button class="bg-gradient-to-r from-gray-700 to-gray-600 hover:from-gray-600 hover:to-gray-500 text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-md transition-all active:scale-95 border border-white/10" @click="$inertia.visit('/art-library-management')">Manage</button>
...
<script setup>
    ...
    const localLibraryTotal = ref(props.libraryTotal);
    
    watch(() => props.ArtLibrary, (newData) => {
        if (newData.current_page === 1) {
            localLibraryTotal.value = props.libraryTotal;
        }
    });
